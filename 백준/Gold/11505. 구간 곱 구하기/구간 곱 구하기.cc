...
void createTree(long long cur, int left, int right)
{
    ...
    tree[cur] = tree[cur * 2] % 1000000007 * (tree[cur * 2 + 1] % 1000000007) % 1000000007; // 배열 트리 방법으로 값 가져오기
}

// cur : 트리상 인덱스. start, end : cur가 담고 있는 범위. left right : 찾는 범위
long long query(long long cur, int start, int end, int left, int right)
{
    if (left > end || right < start) return 1; // 찾는 범위 밖이면 안함
    if (left <= start && end <= right) return tree[cur]; // 범위 전체가 구하는 범위 안이면 다 가져오기

    int mid = (start + end) / 2;
    return query(cur * 2, start, mid, left, right) % 1000000007 *
        (query(cur * 2 + 1, mid + 1, end, left, right) % 1000000007) % 1000000007; // 구간 분할해서 계산
}
...
void update(long long cur, int start, int end, int index, long long value)
{
    ...
    tree[cur] = tree[cur * 2] % 1000000007 * (tree[cur * 2 + 1] % 1000000007) % 1000000007;
}
